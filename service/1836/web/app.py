"""1836 Vic3 问答网页：协议对齐旧 gatekeeper.web，实现不依赖 archive。"""
...
def create_app(
    *,
    ...
    app = FastAPI(title="Patchouli Vic3 QA")
    ...
    async def api_mods(
        ...
        if filt is not None:
            ...
            if raw in {"vanilla", "vanillaonly", "onlyvanilla"}:
                st = "vanillaonly"
            elif raw in {"common", "bytiancao"}:
                st = raw
        if st == "vanillaonly":
            mods: list[dict[str, str]] = []
            cat_filt = None
        elif st in {"common", "bytiancao"}:
...
def serve() -> None:
    ...
    port = max(1, int(os.environ.get("WEB_PORT") or "1836"))
    log.info("1836 web on http://%s:%s", host, port)
