...
DEFAULT_GAME = Path(__file__).resolve().parent
...
def _resolve_corpus_path(
    ...
    # 边界检查不 follow 最终 symlink，避免「视图内合法链接」被误判越界；
    # 同时拒绝 .. 逃逸。
    rel_path = Path(p)
    if rel_path.is_absolute() or ".." in rel_path.parts:
        return {"ok": False, "error": "path_outside_game"}
    fp = root.joinpath(*rel_path.parts)
    try:
        fp.absolute().relative_to(root)
    except ValueError:
        return {"ok": False, "error": "path_outside_game"}
    return fp, root, p
...
def _iter_game_files(root: Path, glob_pat: str | None) -> list[Path]:
    """扫文本类文件：common / events / map_data / tables，以及根下说明文件。"""
    allowed = ["common", "events", "map_data", "tables"]
    text_suffix = {".txt", ".csv", ".md", ".json", ".yml", ".yaml", ".tsv"}
    out: list[Path] = []
    root = root.resolve()

    def _rel_ok(p: Path) -> Path | None:
        try:
    ...
    if glob_pat:
        for p in root.glob(glob_pat):
            if p.is_file() and _rel_ok(p) is not None:
                out.append(p)
        return out

    for name in ("structure.md", "routing_table.json", "query_aliases.json"):
        fp = root / name
        if fp.is_file():
            out.append(fp)
