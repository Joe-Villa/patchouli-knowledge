...
# service/1836 → service → 仓库根；约定：请求日志在 log/1936/
_REPO = _CORE.parent.parent
_DEFAULT_LOG_DIR = _REPO / "log" / "1936"
...
class RequestConfig:
    ...
    auto_select_corpus: bool = False
    """1936 仅原版：默认不按题干切换模组资料库。"""
    ...
    def defaults(cls, *, core: Path | None = None) -> RequestConfig:
        """开发机默认：语料相对服务目录；请求日志约定在仓库 log/1936/。"""
    ...
    def from_mapping(cls, data: Mapping[str, Any], *, base_dir: Path | None = None) -> RequestConfig:
        ...
        auto_select = False if auto_raw is None else bool(auto_raw)
