...
def _ensure_core_paths() -> Path:
    core = Path(__file__).resolve().parents[1]  # service/1936
...
def normalize_corpus(raw: Any) -> dict[str, Any]:
    _MAX_TAGS = 2
    _VALID_STATUSES = frozenset({"common", "vanillaonly"})
...
def list_mods_for_api(
    *,
    ...
    tool_root = Path(__file__).resolve().parents[3] / "tool"
    ...
    for raw in data.get("mods") or []:
        ...
        if want is not None and cat != want:
            continue
        entry = enrich_mod_entry({**raw, "category": cat}, index)
        out.append(entry)
    return sort_mod_entries(out)
